...
다른 엔티티를 받아야 할 때, 
서비스를 통해 받아야 하나, 아니면 리포지토리를 통해 받아야 하나?

1. 서비스를 이용하면, ?
2. 리포지토리를 이용하면, 바로 DB에 접근함. 

> 지금 상황에서 서비스를 이용하는 메리트가 안 느껴짐.  
> 오히려 메서드 1개만 추가되서 더 귀찮아 진다고 생각.  
> 리포지토리를 이용해서 받아야 겠다.  
> (서비스는 그냥 리포지토리 위임받는거라 딱히 필요가 없을 듯)

> 서비스 레이어는 비즈니스 로직을 해결하는 레이어.  
> 엔티티를 받는 것은, 서비스 레이어가 아니라, 리포지토리 레이어가 하는 역할  
> 리포지토리가 맞는 것 같다.

---

Post 방문했을 때, 조회 수 1 증가시키는 것을 어떻게 구현하나?

글 1개를 방문할 때마다, 조회 수를 늘려야 하는데, (나중에 방문기록으로도 필요)  
30분?15분? 제한도 있어야 함

1. 그냥 get 요청할 때마다 조회 수 1 늘림(제한 시간 적용 불가 - X)
2. get 요청한 IP를 서버에 저장시켜서 조회 체크(기능 구현은 가능하지만, 쿼리 요청이 너무 잦아진다)
3. 액세스 토큰으로 방문 구분, 만료되면 다시 조회 수 1 증가(로그인 안 한 클라이언트도 체크해야 해서 불가 - X)
4. 구글 Analytics 사용

---

@Builder 생성자에 추가하면 어떤식으로 동작하지?, 여러 생성자에 여러 @Builder를 추가하면 어떻게 되나?

---

DB에서 나노초로 관리하면 오류남

2023/12/14.23:59:59.999999를 넣으면, 2023/12/15.00:00:00.000000으로 저장됨

> DB는 나노초에 대해서 반올림한다고 함
>
> 테스트를 할 때나, 로직 짤 때, DB에서 시간 관리는 나노초가 아니라, 초 단위로 하기로 결정